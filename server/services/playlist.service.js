...
class PlaylistService {
  ...
  /**
   * DONE : Implémenter la récupération de toutes les playlists
  ...
  async getPlaylistById (id) {
    const query = { id };
    const playlist = (await this.collection.find(query).toArray())[0];
    if (!playlist) { return null; }
    return playlist;
  }

  /**
   * DONE : Implémenter l'ajout d'une nouvelle playlist
  ...
  /**
   * DONE : Implémenter la mise à jour d'une playlit existante
  ...
  async updatePlaylist (playlist) {
    ...
    await this.collection.updateOne(filter, { $set: { id: playlist.id } });
    await this.collection.updateOne(filter, { $set: { name: playlist.name } });
    await this.collection.updateOne(filter, { $set: { description: playlist.description } });
    await this.collection.updateOne(filter, { $set: { songs: playlist.songs } });
  }
  ...
  /**
   * DONE : Implémenter la recherche pour les 3 champs des playlists. Astuce : utilisez l'opérateur '$or' de MongoDB
   *
  ...
  async search (substring, exact) {
    ...
    const filter2 = { description: { $regex: `${substring}`, $options: "i" } };

    const filterExact = { name: { $regex: `${substring}` } };
    const filterExact2 = { description: { $regex: `${substring}` } };

    if (exact) {
      const playlists = await this.collection.find({ $or: [filterExact, filterExact2] }).toArray();
      return playlists;
    }
