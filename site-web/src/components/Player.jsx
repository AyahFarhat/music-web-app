...
export default function Player() {
  ...
  // DONE : ajouter une action de jouer la prochaine chanson
  const playNextSong = () => {
    dispatch({type: ACTIONS.NEXT});
  };

  // DONE : ajouter une action de jouer la chanson précédante
  const playPreviousSong = () => {
    dispatch({type: ACTIONS.PREVIOUS});
  };

  // DONE : ajouter une action de déplacement dans la barre de progrès
  const seek = (newTime) => {
    dispatch({type: ACTIONS.SEEK, payload: { time: newTime }});
  };

  // DONE : ajouter une action d'avancement/recul dans la chanson
  const scrubTime = (delta) => {
    dispatch({type: ACTIONS.SCRUB, payload: { delta: delta }});
  };

  // DONE : ajouter une action de fermer/ouvrir le son
  const muteToggle = () => {
    dispatch({type: ACTIONS.MUTE})
  };

  // DONE : ajouter une action d'activer ou désactiver le mode "shuffle"
  const shuffleToggle = () => {
    ...
    <>
      ...
      <div id="controls" className="flex-column">
        ...
        <section id="timeline-container" className="flex-row">
          {/*DONE : afficher le temps en cours de la chanson */}
          <span id="timeline-current">{formatTime(state.audio.currentTime)}</span>
