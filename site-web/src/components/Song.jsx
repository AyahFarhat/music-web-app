...
export default function Song({ song, index }) {
  ...
  // DONE : envoyer une demande de modification au serveur et mettre l'interface à jour.
  ...
  // DONE : envoyer une action PLAY avec le bon index au reducer.
  const playSong = () => {
  ...
  return (
    ...
    >
      ...
      {/*DONE : ajouter les bonnes informations de la chanson */}
     
      <p>{song.name}</p>
      <p>{song.genre}</p>
      <p>{song.artist}</p>

      {/*DONE : modifier le statut aimé seulement si index n'existe pas */}
      <button
